...
#Some sort of chi sqr test on these?
...
There is no meaningful interpretation of the log-likelihoods on their own, but they can be used to compare models. They are related to the sum squares. Of the three models, Model 3 has the largest log likelihood and thus is relatively the best fit of the three models. 

c)The anova comparing the models gives a p value of 0.008. This means that there is a significant difference between the models. 

d)Age should be included in the model. The AIC for the employment + age model is smaller than for the employment model. 

Age should be included in the model. If we are happy with using the log likelihood of the model with employment included, then we must also be happy with the model with age included since it is greater than the model with just employment. 
...
AIC.e <- 2*2 - 2 *logLik(q3.cox.e)
AIC.e

#no intercept, group, and age
AIC.a <- 2*1 - 2 * logLik(q3.cox.a)
AIC.a

#added age as a parameter, so 2 + 1 = 3
AIC.a.e <- 2*3 - 2 * logLik(q3.cox.a.e)
AIC.a.e
#confirm that it's the same. 
AIC(q3.cox.a.e, k = 2)

AIC.uni <- 2*0 - 2* logLik(q3.cox)
AIC.uni
AIC(q3.cox, k = 2)
...
#repeat with categorical age 
q5.full.model.agecat <- coxph(data = q2.data, formula = q2.surv.object ~ . - id - ttr - relapse - ageGroup2 - ageGroup4 - age)
...
If age is dichotomous, the hazard ratio for the age variable is 2.41 (95% CI of 1.51, 3.86). This means that being under the age of 50 is associated with a shorter time to relapse, a 141% increased risk of relapse. Friggen kids these days. 
...
After age 60 the confidence intervals get really large. Under age 30 also seems to have fairly large 95% CI. If we look at the histogram of age, we see that the age data looks fairly normal, but there is a long right tail. We have a few data points above age 70 that could be influencing the stability of our estimates on either end of out regression. Superficially to log(hazard) vs age plot looks non-linear, but I'm skeptical. I think it is linear with some outliers above age 70 that are ripping the 95% CI wide open at either end. Notice how you can trace a linear line within the 95% CI.
...
b)Most of the values don't have a huge influence on the model, however there is one value whose influence on the model coefficients is nearly 3 times greater than the next "biggest influencer". It's like Huda Kattan decided to be in this model instead of instagram. It's the 87th row of the data frame.
...
q9.data$pfsmo <- q9.data$pfs/30

q9.surv.obj <- Surv(q9.data$pfsmo)

q9.cox <- coxph(q9.surv.obj ~ q9.data$stage)
...
survfit <- survfit(q9.surv.obj ~ q9.data$stage)
...
plot(survfit(q9.surv.obj ~ q9.data$stage), col = c("blue", "red"), xlab = "Survival Time (months)", fun = "cloglog")
