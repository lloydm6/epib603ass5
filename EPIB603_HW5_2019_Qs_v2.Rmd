...
#d) Repeat step b), but now include age as a potential confounder. Discuss if the age differences between the groups explain some or all of any observed treatment differences. (2 points)
q2.d.curvetest <- survdiff(q2.surv.object ~ q2.data$grp + strata(q2.data$ageReg))
q2.d.curvetest
...
q3.cox.a <- coxph(q2.surv.object ~ q2.data$age)
summary(q3.cox.a)

q3.cox.a.e <- coxph(q2.surv.object ~ q2.data$age + q2.data$employment)
summary(q3.cox.a.e)
...
d)Age should be included in the model. The inclusion of age in the employment model increases the log likelihood compared to the employment only model ($\chi^2$ test comparing log likelihoods gives a p value of `r q3.chi.evae`, so reject the null hypothesis that they are the same). Comparing the log likelihoods of the age only model to the age and employment model also shows that there is an improvement (p value of `r q3.chi.avae`). We can also just eyeball it and see that the log likelihood of the age and employment model (-379) is larger (less negative) than either the age only (-383) or employment only models (-385). 
...
AIC.a <- round(2*1 - 2 * logLik(q3.cox.a)[1], 2)
AIC.a



#added age as a parameter, so 2 + 1 = 3
AIC.a.e <- round(2*3 - 2 * logLik(q3.cox.a.e)[1], 2)
AIC.a.e
...
<div id="body">
**Type your answer here:** 
a) The AIC of the model without covariates is `r AIC.uni`. The AIC of the age only model is `r AIC.a`. The AIC of the employment only model is `r AIC.e`. The AIC of the age + employment model is `r AIC.a.e`. Note age as a continuous variable was used. 

b) It is Akaikies Information Criterion that is meant to communicate the goodness of fit of a model. It has two important terms, one that is a measure of fit based on the sum squares of errors of prediction (SSE, aka residual sum of squares) and another that adds a penalty for each parameter added to the model. The penalty is meant to prevent over fitting. When comparing models, the model with the smallest AIC should be considered the best, though some argue that the AIC penalty for additional parameters is not strong enough and blindly chasing the lowest AIC will not prevent overfitting. For this question, we have a lot of observations compared to the number of covariates we are looking at, so I'm not too worried about over fitting at the mo.
...
a)The most parsimonious model includes the treatment group, age (continuous), and employment variables. We also repeated the process using the dichotomous age categories and came to the same result.

b)If age has 4 categories, the hazard ratio for the 35-49 age variable is 0.89 (95% CI of 0.48, 1.68) and suggests 35-49 year olds have a 11% lower risk of relapse compared to 21-34 year olds, though the HR 95% CI crosses the null in a big way and is thus inconclusive. The hazard ratio for the 50-64 age variable is 0.36 (95% CI of 0.18, 0.73) and suggests 50-64 year olds have a 64% lower risk of relapse compared to 21-34 year olds with the 95%CI being well below the null. The hazard ratio for the 65+ age variable is 0.49 (95% CI of 0.18, 1.32) and suggests 65+ year olds have a 51% lower risk of relapse compared to 21-34 year olds, though the HR crosses the null and is thus inconclusive, but may be interesting because a large part of the 95% CI is below 1. These hazard ratios are all in comparison to the reference category of 21-34 years old. 

If age is continuous, the hazard ratio for the age variable is 0.97 (95% CI of 0.95, 0.99). This means that for increase in age, there is an associated longer time to relapse, every increase in age by 1 year has a 3% reduced risk of relapse. The upper bound of the 95% CI suggests this reduction in risk may be as little as 1%, but that is per year of age, so a 10 year age difference could mean an important difference in risk. 

If age is dichotomous, the hazard ratio for the under 50 age variable is 2.41 (95% CI of 1.51, 3.86). This means that being under the age of 50 is associated with a shorter time to relapse, a 141% increased risk of relapse. Friggen kids these days. 
...
After age 60 the confidence intervals get really large. Under age 30 also seems to have fairly large 95% CI. If we look at the histogram of age, we see that the age data looks fairly normal, but there is a long right tail. We have a few data points above age 70 that could be influencing the stability of our estimates on either end of out regression. At first glance, the log(hazard) vs age plot looks non-linear, but the large CIs at either end don't rule out linearity. Notice how you can trace a nice, straight, linear line within the 95% CI. We cannot establish that the relationship has departed from linearity. 
...
index.obs <- 1:num.obs
plot(q8.resid[,4] ~index.obs, type="h", xlab="Observations", ylab="Change in coefficients")
abline(h=0)
...
b)Most of the values don't have a huge influence on the model, however there is one value whose influence on the model coefficients is nearly 3 times greater than the next "biggest influencer". It's like Huda Kattan left Instagram and decided to be in this model. It's the 87th row of the data frame.
...
#KM plot
plot(survfit(q9.surv.obj ~ q9.data$stage), col = c("blue", "red"), xlab = "Survival Time (months)")
legend(x = "topright", inset = c(0.05, 0.05), col = c("red", "blue"), lty = c(1, 1), legend = c("Mestatic", "Locally Advanced"))

#Cox PH plot...i think. 
plot(survfit(q9.cox), col = c("blue", "red"), xlab = "Survival Time (months)")
legend(x = "topright", inset = c(0.05, 0.05), col = c("red", "blue"), lty = c(1, 1), legend = c("Mestatic", "Locally Advanced"))


#pull vectors that I want out of survfit, build it in ggplot, for some reason they get ordered...which is nice
survfit <- survfit(q9.cox)
log.time <- log(survfit$time)
log.log.prob <- log(-log(survfit$surv))
survfit$n

#number at risk, this shows the two stages, for some reason, three of the M have disappeared. Too tired to GAF at the mo
survfit$n.risk
survfit$strata

#define type again, they are in order and LA comes first. For some reason 
type <- c(rep("LA", 8), rep("M", 30))
log.df <- data.frame(log.log.prob, log.time, type)

#make plot
library(ggplot2)
ggplot(data = log.df, aes(x = log.time, y = log.log.prob, col = type)) + geom_step()

#now flip it for reasons that are super clear to me
ggplot(data = log.df, aes(x = log.time, y = -log.log.prob, col = type)) + geom_step()

plot(survfit(q9.cox), col = c("blue", "red"), xlab = "Survival Time (months)", fun = "cloglog")
legend(x = "topright", inset = c(0.05, 0.75), col = c("red", "blue"), lty = c(1, 1), legend = c("Mestatic", "Locally Advanced"))
...
c)I think he assumption is violated. We have lines crossing. The hazards are not proportional. 
...
b)The p value is `r q.10.pval`, which is evidence that there is a relatioship between the residuals and time. There also appears to be a pattern between residuals and time on the plot so this is further evidence that the proportional hazard assumption is violated. To address this, we may need to look for an interaction term of one of the covariates with time or stratify by some other covariate. 
